...
public class ClassFinder {
    ...
    public Set<Class<?>> findClassesWhichExtend(String className) throws MojoFailureException {
        ...
        try {
            ...
            return newHashSet((Iterable<? extends Class<?>>) findClassesWhichExtend(superClass, classLoader));
    ...
    private URLClassLoader getDependencyClassLoader() throws MojoFailureException {
        Set<Artifact> artifacts = new HashSet<>(project.getDependencyArtifacts());
        artifacts.add(project.getArtifact());
        List<URL> urls = artifacts.stream().map(Artifact::getFile).filter(f -> f != null).map(f -> toURL(f)).collect(toList());
