...
public class ExtendsPojoTest {

    @Rule public FixtureRule fixtureRule = FixtureRule.initFixtures();

    @Fixture boolean booolVal;
    @Fixture String stringVal;
    ...
    public void string_field_should_be_matchable() {
        pojo = newBuilder().withStringField(stringVal).build();

        assertThat(pojo, newMatcher().withStringField(stringVal));
        assertThat(pojo, newMatcher().withStringField(is(stringVal)));
    ...
    public void listOfStrings_should_be_matchable() {
        pojo = newBuilder().withListOfStrings(asList(stringVal, stringVal1)).build();

        assertThat(pojo.getListOfStrings(), contains(stringVal, stringVal1));

        assertThat(pojo, newMatcher().withListOfStrings(asList(stringVal, stringVal1)));
        assertThat(pojo, newMatcher().withListOfStrings((Matcher) contains(stringVal, stringVal1)));

        assertThat(pojo, not(newMatcher().withListOfStrings(asList(otherStringVal, stringVal))));
    ...
    public void listOfAnything_should_be_matchable() {
        List<Object> listVal = asList(optionalIntegerVal, stringVal, integerVal1);
    ...
    @Test
    public void extensionField_should_be_matchable() {
        pojo = newBuilder().withExtensionField(integerVal1).build();

        assertThat(pojo.getExtensionField(), is(integerVal1));

        assertThat(pojo, newMatcher().withExtensionField(integerVal1));
        assertThat(pojo, newMatcher().withExtensionField((Matcher) is(integerVal1)));

        assertThat(pojo, not(newMatcher().withExtensionField(otherIntegerVal)));
        assertThat(pojo, newMatcher().withExtensionField(not(is(otherIntegerVal))));
    }

    protected ExtendsPojoMatcher newMatcher() {
        return new ExtendsPojoMatcher();
    }

    protected ExtendsPojoBuilder newBuilder() {
        return new ExtendsPojoBuilder();
    }
